...
export async function POST(request: NextRequest) {
  try {
    ...
    const paymentIntentData: Stripe.PaymentIntentCreateParams = {
      ...
      shipping: {
        ...
        address: {
          ...
          // FIX: default to Canada, not US
          country: donor_info.country || "CA",
        },
      },
    }
    ...
    if (confirmedPaymentIntent.status === "requires_action") {
      ...
      // If monthly donation, create customer + subscription
      if (frequency === "monthly") {
        try {
          ...
          // FIX: trial_end set to 30 days from now prevents double-charge.
          // Month 1 is already covered by the PaymentIntent confirmed above.
          // The subscription's first invoice will not be generated until day 30.
