...
export const IconCard: React.FC<IconCardProps> = ({
  ...
  const content = (
    <div
      className={`
        flex flex-row ${gapClasses[gap]} items-center
    ...
    >
      ...
      >
        {imageUrl && (
          <img src={imageUrl || "/placeholder.svg"} alt={heading} className="w-full h-full object-cover rounded-full" />
        )}
      </div>
      <div className="flex flex-col justify-center gap-1 min-w-0 flex-1" style={{ marginTop: '4px' }}>
        {heading && (
          <h3
            className={`font-bold text-3xl leading-tight ${alignClasses[headingAlign]}`}
        ...
        {text && (
          <p className={`text-xl leading-relaxed ${alignClasses[textAlign]}`} style={{ color: textColor }}>
            {text}
          </p>
        )}
      </div>
